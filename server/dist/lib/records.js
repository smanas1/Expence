...
function titleCaseRecordName(value) {
...
export function normalizeRecordPayload(input) {
...
export async function ensureRecordsBackfilledForUser(userId) {
    ...
    const recordIdsBySection = new Map();
    let colorIndex = 0;
    ...
    await Promise.all(legacyTransactions.map((transaction) => {
        const section = typeof transaction.section === "string" && transaction.section.trim() ? transaction.section.trim() : "self";
        const recordId = recordIdsBySection.get(section);
        if (!recordId) {
            return Promise.resolve();
        }
        return TransactionModel.updateOne({ _id: transaction._id }, {
            $set: {
                recordId,
                section,
            },
