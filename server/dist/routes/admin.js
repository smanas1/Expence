...
adminRouter.patch("/users/:id", async (req, res) => {
    ...
    const { name, email, currency, role, password } = req.body;
    const update = {};
    if (name?.trim()) {
...
adminRouter.get("/transactions", async (req, res) => {
    const { q = "", kind, section, userId } = req.query;
    const query = {};
    if (kind && kind !== "all") {
        query.kind = kind;
    }
...
adminRouter.patch("/donations/:id/status", async (req, res) => {
    ...
    const donation = await DonationPlanModel.findByIdAndUpdate(donationId, {
        $set: {
            status: nextStatus,
