...
recordsRouter.use(async (req, _res, next) => {
...
recordsRouter.get("/:id", async (req, res) => {
    ...
    const categoryBreakdown = (aggregate?.categories ?? []).map((item) => ({
...
recordsRouter.patch("/:id", async (req, res) => {
    ...
    const record = await RecordModel.findOneAndUpdate({ _id: new mongoose.Types.ObjectId(recordId), userId }, { $set: payload }, { returnDocument: "after" }).lean();
    if (!record) {
        res.status(404).json({ message: "Record not found." });
        return;
    }
